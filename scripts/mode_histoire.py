...
def Histoire(jeu,story):
    pygame.init()
    lenivo = stage_liste[story.stage]
    ...
    while not gameExit and jeu.selected == "histoire" :
        ...
        if story.stage <= 10:
            ...
            jeu.selected = "accueil_histoire"
            story.unlock_perso = jeu.unlock_perso
            story.unlock_perso2 = jeu.unlock_perso2
            story.unlock_nivo = jeu.unlock_nivo
            m = random.choice(random_liste)
            pygame.mixer.music.set_volume(jeu.vol_music_menu)
            pygame.mixer.music.load ( start_playlist[m])
            pygame.mixer.music.play(-1) 
